...
import { getAllVehicles, getBrands, getCatalogDate, getModels } from "@/data/catalog";
import { isVersionIndexable, versionsOf } from "@/data/catalog/selectors";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Images des guides et articles (fichiers réels de public/editorial, affichés dans la page).
  const heroImages = (page: { hero?: { src: string }; sections: { image?: { src: string } }[] }) => {
    const srcs = [page.hero?.src, ...page.sections.map((s) => s.image?.src)].filter((x): x is string => Boolean(x));
    return srcs.length ? srcs.map((x) => `${base}${x}`) : undefined;
  };
  ...
  const brandRoutes = brands
    .filter((b) => models.filter((m) => m.brandSlug === b.slug).length >= 2)
    .map((b) => entry(`/voitures-electriques/${b.slug}`, catalogDate, "monthly", 0.6));

  const modelRoutes = models.map((m) => entry(vehicleHref(m), new Date(m.source.lastUpdated), "monthly", 0.7));

  const versionRoutes = allVehicles
